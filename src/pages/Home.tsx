...
const sliderImages = [
  ...
  {
    url: "https://images.pexels.com/photos/1091294/pexels-photo-1091294.jpeg?auto=compress&cs=tinysrgb&w=600",
    title: "PHOTOGRAPHY STUDIO"
  },
  {
    url: "https://images.pexels.com/photos/16249347/pexels-photo-16249347/free-photo-of-a-man-in-a-sweater-looking-up.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1",
    title: "PHOTOGRAPHY STUDIO"
  }
];
...
const features = [
  {
    ...
    description: "Our team consists of seasoned photographers with years of experience in capturing life's precious moments.",
    stats: "10+ Years",
    accent: "bg-blue-50"
  },
  {
    ...
    description: "We use the latest photography equipment to ensure the highest quality images for our clients.",
    stats: "1000+ Clients",
    accent: "bg-purple-50"
  },
  {
    ...
    description: "We tailor our services to meet the unique needs of each client, ensuring a personalized experience.",
    stats: "500+ Events",
    accent: "bg-rose-50"
  }
];
...
<div className="flex justify-center">
  <a href="/contact">
    <motion.button whileHover={{ scale: 1.05 }} className="mt-8 bg-black text-white px-6 py-3 md:px-8 md:py-4 text-lg transition-all duration-300 hover:bg-gray-800 rounded-full flex items-center gap-2 group">
      Contact Us
        ...
        <div className="w-full md:w-1/2 h-[calc(130vh-6rem)] relative overflow-hidden">
          ...
          <div className="absolute bottom-5 md:bottom-10 right-5 md:right-10 flex gap-3 z-10">
            {sliderImages.map((_, index) => (
              <button key={index} onClick={() => setCurrentSlide(index)} className={`h-2 rounded-full transition-all duration-300 ${currentSlide === index ? 'w-8 bg-white' : 'w-2 bg-white/50'}`} />
            ))}
          </div>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto text-center">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {features.map((feature, index) => (
              <motion.div key={index} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.2 }} className={`p-6 md:p-8 rounded-2xl ${feature.accent} hover:shadow-xl transition-all duration-300`}>                
